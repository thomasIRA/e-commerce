      ...
      <div class="container px-4 px-lg-5 my-5">
        <div class="flex flex-col lg:flex-row items-center lg:items-start">
          <div class="lg:w-1/2 mb-5 lg:mb-0 p-6">
            <img class="w-full mb-5 lg:mb-0" :src="product.photo" :alt="product.name" :title="product.name"/>
          </div>
          <div class="lg:w-1/2 p-6">
            ...
            <div class="flex items-center">
              <input type="number" v-model.number="product.qty" class="mr-6 w-[80px] bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
              <button @click="add(product.name, productIndex)" class="mr-6 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Add to cart</button>
            </div>
            <hr class="my-4">
            <div class="flex">
...
export default {
  ...
  computed: {
    product () {
      return this.inventory.find(p => p.id === Number(this.$route.params.id))
    },
    productIndex () {
      return this.inventory.findIndex(p => p.id === Number(this.$route.params.id))
    }
  }
}
</script>
